...
def write_blank_line_to_csv():
    ...
    with open('4colorsresult.csv', mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([])

def write_to_csv(lab_color1, lab_color2, rgb1, rgb2, measured_luminance, angular_velocity):
    """Write data to a CSV file."""
    file_exists = os.path.isfile('4colorsresult.csv')
    with open('4colorsresult.csv', mode='a', newline='') as file:
        writer = csv.writer(file)
        if not file_exists:
            writer.writerow(
        ...
        writer.writerow([lab_color1[0], measured_luminance, rgb1, lab_color1,
                         lab_color2[0], measured_luminance, rgb2, lab_color2,
                         format(angular_velocity, '.3f')])



def select_luminance_and_colors():
    # Step 1: Pick a random MeasuredL value in the interval of 10 to 210
    measured_luminance = random.uniform(10, 210)

    # Step 2: Determine which color sets have MeasuredL covering this random value
    available_colors = []
    if min(df['MeasuredL1']) <= measured_luminance <= max(df['MeasuredL1']):
        available_colors.append('color1')
    if min(df['MeasuredL2']) <= measured_luminance <= max(df['MeasuredL2']):
        available_colors.append('color2')
    if min(df['MeasuredL3']) <= measured_luminance <= max(df['MeasuredL3']):
        available_colors.append('color3')
    if min(df['MeasuredL4']) <= measured_luminance <= max(df['MeasuredL4']):
        available_colors.append('color4')

    # Step 3: Randomly pick 2 colors from the available options
    chosen_colors = random.sample(available_colors, 2)

    # Step 4: Find their L* value, and corresponding a*, b* values
    lab_colors = []
    for color in chosen_colors:
        if color == 'color1':
            L_star = get_l_values(measured_luminance)[0]
            a_star = random.choice(df['a1*'])
            b_star = random.choice(df['b1*'])
        elif color == 'color2':
            L_star = get_l_values(measured_luminance)[1]
            a_star = random.choice(df['a2*'])
            b_star = random.choice(df['b2*'])
        elif color == 'color3':
            L_star = get_l_values(measured_luminance)[2]
            a_star = random.choice(df['a3*'])
            b_star = random.choice(df['b3*'])
        elif color == 'color4':
            L_star = get_l_values(measured_luminance)[3]
            a_star = random.choice(df['a4*'])
            b_star = random.choice(df['b4*'])

        lab_colors.append((L_star, a_star, b_star))

    lab_color1, lab_color2 = lab_colors

    return lab_color1, lab_color2, measured_luminance
...
def main():
    ...
    lab_color1, lab_color2, measured_luminance = select_luminance_and_colors()
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                write_to_csv(lab_color1, lab_color2, g1, g2, measured_luminance, angular_velocity)
                ...
                if event.key == pygame.K_LEFT:
                    ...
                    angular_velocity = random.choice([i * 0.1 for i in range(5, 21)] + [i * 0.1 for i in range(-20, -5)])
                    #angular_velocity = 0
        ...
        # Draw small gray circles
        # Draw small gray circles
        # Define the color with alpha transparency (128, 128, 128, 128) means semi-transparent gray
        transparent_gray = (128, 128, 128, 200)

        # Create a surface with per-pixel alpha
        circle_surface = pygame.Surface((width, height), pygame.SRCALPHA)

        # Draw small gray circles on the surface
        for yc in range(r_max, height, 2 * r_max):
            for xc in range(r_max, width, 2 * r_max):
